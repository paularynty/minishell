...
//draft of new cd approach below
...
//  * Returns TRUE on success or FALSE if `getcwd()` fails.
//  */
// static int	update_pwd(t_mini *shell, char *cwd)
// {
// 	char	*pwd;
...
// 		free(shell->cwd);
// 		shell->cwd = cwd;
// 		env_unset_variable(shell->env, "OLDPWD");
// 		return (TRUE);
// 	}
// 	if (!env_set_variable(shell, "PWD", cwd)
// 		|| !env_set_variable(shell, "OLDPWD", shell->cwd))
// 	{
// 		free(cwd);
// 		return (FALSE);
// 	}
...
// 	if (!old_pwd)
// 	{
// 		error_builtin(CD, "check 1", "OLDPWD not set");
// 		return (FALSE);
// 	}
// 	new_pwd = env_get_variable(shell->env, "OLDPWD");
// 	if (!new_pwd)
// 		return (error_builtin(CD, "check 2", "OLDPWD not set"), FALSE);
// 	printf("%s\n", new_pwd);
// 	if (chdir(new_pwd) == -1)
// 		return (error_builtin(CD, new_pwd, NULL), FALSE);
// 	if (!env_set_variable(shell, "OLDPWD", old_pwd))
// 	{
// 		free(old_pwd);
// 		free(new_pwd);
...
// static int	cd_options(t_mini *shell, char **args, char *cwd)
// {
// 	if (!args[1])
// 	{
// 		if (!cd_home(shell))
